...
{
    HAL_Delay_nMs( 20 );
   // GpioWrite( SX_RST_GPIO_Port, SX_RST_Pin, 0 );
    HAL_GPIO_WritePin(SX_RST_GPIO_Port, SX_RST_Pin, GPIO_PIN_RESET);
    HAL_Delay_nMs( 50 );
    //GpioWrite( SX_RST_GPIO_Port, SX_RST_Pin, 1 );
...
{
    return /*( GPIO_ReadInputDataBit( RADIO_DIOx_PORT, RADIO_DIOx_PIN ) << 1 ) |*/ ( GPIO_ReadInputDataBit( SX_BUSY_GPIO_Port, SX_BUSY_Pin ) << 0 );
}
